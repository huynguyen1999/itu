...
public actor OfflineStore {
    ...
    public init(accountID: String = "anonymous", location: OfflineStoreLocation) {
        let safeAccountID = accountID.replacingOccurrences(
            of: "[^A-Za-z0-9_-]",
            with: "_",
            options: .regularExpression
        )
        fileURL = location.rootURL.appendingPathComponent("offline-\(safeAccountID)-v1.json")
        encoder = JSONEncoder()
    ...
    public init(accountID: String = "anonymous", baseURL: URL? = nil) {
        let root = baseURL ?? FileManager.default
            .urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("iTu", isDirectory: true)
        self.init(accountID: accountID, location: OfflineStoreLocation(rootURL: root))
    }

    public func load() throws -> OfflineSnapshot {
    ...
    public func outboxEvents() -> AsyncStream<OutboxEvent> {
        let id = UUID()
    ...
    public func upsertFocusSession(_ session: FocusSession) {
    ...
    public func appendMutation(_ mutation: SyncMutation) {
        outboxChanged = true
