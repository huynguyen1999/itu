    ...
    func reapplyPendingJournalMutations() {
        ...
        for mutation in state.mutations where mutation.kind.hasPrefix("journal.") || mutation.kind == "journal_revision.restore" {
            ...
            case "journal.update", "journal_revision.restore":
                ...
                let templateID = offlineJournalSnapshotTemplateID(mutation.payload)
                if templateID.present { state.journalNotes[index].templateId = templateID.value }
                if let tagIDs = offlineJournalSnapshotTagIDs(mutation.payload) { state.journalNotes[index].tagIds = tagIDs }
...
private func offlineJournalSnapshotTagIDs(_ snapshot: [String: JSONValue]) -> [String]? {
...
private func offlineJournalSnapshotTemplateID(_ snapshot: [String: JSONValue]) -> (present: Bool, value: String?) {
