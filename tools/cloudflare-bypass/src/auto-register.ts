  ...
  for (const item of planResult.plan) {
    ...
    if (item.status !== REGISTRATION_STATUS.READY && item.status !== REGISTRATION_STATUS.MULTIPLE_MATCHES) {
      results.push({
        ...
        time: (slot?.startTime || item.request?.time || 'N/A').slice(0, 5),
        teacher: slot?.teacherNickName || slot?.teacherName || item.request?.teacher || 'N/A',
        room: slot?.room || 'N/A',
        message: item.reason,
      });
      continue;
...
export async function runAutoRegister(options: AutoRegisterOptions = {}): Promise<AutoRegisterRunResult> {
  ...
  // 3. Determine course list: prefer inline courses, fall back to file
  let courseList: CourseCriteria[] = [];
  if (Array.isArray(options.courses) && options.courses.length > 0) {
    console.log(`📋 [Step 2/5] Using ${options.courses.length} inline course criteria from request body.`);
    courseList = options.courses;
  } else {
    // Only load from file if there are courses to register
    // (skip entirely when the run is cancel-only)
