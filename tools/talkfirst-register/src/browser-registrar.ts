...
import { getMondayOfWeek, getSundayOfWeek, normalize } from './course-matcher.ts';
...
async function registerSingleCourse(
  ...
  // 8. Evaluate result from API response, toast notification, modal text, and grid state
  ...
  const modalText = (await modalDialog.innerText().catch(() => '')).trim();
  ...
  const isToastSuccess =
    ...
    (toastText.toLowerCase().includes('thành công') ||
      ...
      toastText.toLowerCase().includes('hoàn tất'));

  const isModalSuccess =
    modalText &&
    (modalText.toLowerCase().includes('thành công') ||
      modalText.toLowerCase().includes('success') ||
      modalText.toLowerCase().includes('đã đăng ký') ||
      modalText.toLowerCase().includes('registered') ||
      modalText.toLowerCase().includes('hoàn tất') ||
      modalText.toLowerCase().includes('bạn đã đăng ký') ||
      modalText.toLowerCase().includes('chúc mừng'));
  ...
  const isModalError =
    modalText &&
    (modalText.toLowerCase().includes('thất bại') ||
      modalText.toLowerCase().includes('không thành công') ||
      modalText.toLowerCase().includes('lỗi') ||
      modalText.toLowerCase().includes('đủ số lượng') ||
      modalText.toLowerCase().includes('trùng') ||
      modalText.toLowerCase().includes('hết chỗ'));

  // 1st Priority: Successful API response, success toast, or success text inside modal
  if (successfulApi || isToastSuccess || isModalSuccess) {
    const successMsg =
      toastText ||
      (isModalSuccess ? 'Registration confirmed in dialog!' : null) ||
      successfulApi?.data?.message ||
      'Registered successfully via API submission!';
    console.log(`   🎉 [Browser] Successfully registered: "${lesson}"! (${successMsg})`);

    // Dismiss modal if close button exists
    const closeBtn = modalDialog
      .locator('button:has-text("Close"), button:has-text("Đóng"), button:has-text("OK"), button:has-text("Done"), button[aria-label="Close"]')
      .first();
  ...
  // 2nd Priority: Error API response, error toast, or error in modal
  if (errorApi || isToastError || isModalError) {
    const errMsg =
  ...
  // 4th Priority: Live API verification check against TalkFirst registered-list
  try {
    const apiClient = new TalkFirstApiClient();
    const monday = getMondayOfWeek(date);
    const sunday = getSundayOfWeek(monday);
    const checkRes = await apiClient.fetchRegisteredClasses({ startDate: monday, endDate: sunday, type: 'All' });
    const isEnrolledInApi = (checkRes.flexibleClass || []).some((item: RegisteredFlexibleClassItem) => {
      const s = item.flexibleClassSchedule;
      const sDate = s?.date?.slice(0, 10);
      const sTime = s?.slotTime?.startTime?.slice(0, 5);
      return (
        sDate === date &&
        (!time || sTime === time.slice(0, 5) || normalize(s?.lesson?.lesson).includes(normalize(lesson)))
      );
    });
    if (isEnrolledInApi) {
      console.log(`   🎉 [Browser] Verified via campus API: "${lesson}" on ${date} is enrolled!`);
      return {
        status: REGISTRATION_STATUS.REGISTERED,
        lesson,
        date,
        ...
        teacher,
        room,
        message: 'Registered successfully (verified via campus API)!',
      };
    }
  } catch (checkErr) {
    console.debug('[Browser] Post-registration API check skipped:', checkErr);
  }

  // If modal still open and no API responses or verification confirmed
...
/**
 * Accurately determines if a course or targetWeek belongs to the current week ("This Week")
 * vs next week ("Next Week").
 */
export function determineIsThisWeek(
  courseDate?: string,
  targetWeek?: string,
  options?: BrowserRegisterOptions
): boolean {
  if (options?.thisWeek || targetWeek === WEEK_MODES.THIS || targetWeek === 'current') {
    return true;
  }
  if (options?.nextWeek || targetWeek === WEEK_MODES.NEXT) {
    return false;
  }
  const thisMondayStr = getMondayOfWeek(new Date());
  if (targetWeek && /^\d{4}-\d{2}-\d{2}$/.test(targetWeek)) {
    return getMondayOfWeek(targetWeek) === thisMondayStr;
  }
  if (courseDate && /^\d{4}-\d{2}-\d{2}$/.test(courseDate)) {
    return getMondayOfWeek(courseDate) === thisMondayStr;
  }
  return false;
}
...
async function runCourseRegistrationInTab(
  ...
  try {
    ...
    const isThisWeek = determineIsThisWeek(course.date, targetWeek, options);
    const weekButtonRegex = isThisWeek ? /This Week|Tuần này/i : /Next Week|Tuần sau/i;

    const weekBtn = page.locator('button').filter({ hasText: weekButtonRegex }).first();
...
async function runSingleCourseCancellationInTab(
  ...
  try {
    ...
    const isModalSuccess =
      ...
      (modalText.toLowerCase().includes('thành công') ||
        modalText.toLowerCase().includes('success') ||
        modalText.toLowerCase().includes('đã hủy') ||
        modalText.toLowerCase().includes('cancelled') ||
        modalText.toLowerCase().includes('hủy thành công'));

    if (successfulCancelApi || isToastSuccess || isModalSuccess) {
      const successMsg =
        toastText ||
        (isModalSuccess ? 'Cancellation confirmed in dialog!' : null) ||
        successfulCancelApi?.data?.message ||
        'Class cancelled successfully via API!';
    ...
    // Verify cancellation via campus API
    try {
      const apiClient = new TalkFirstApiClient();
      const monday = getMondayOfWeek(date);
      const sunday = getSundayOfWeek(monday);
      const checkRes = await apiClient.fetchRegisteredClasses({ startDate: monday, endDate: sunday, type: 'All' });
      const isStillEnrolled = (checkRes.flexibleClass || []).some((item: RegisteredFlexibleClassItem) => {
        const s = item.flexibleClassSchedule;
        const sDate = s?.date?.slice(0, 10);
        const sTime = s?.slotTime?.startTime?.slice(0, 5);
        return (
      ...
      if (!isStillEnrolled) {
        console.log(`   🎉 [Cancel Tab #${tabIndex + 1}] Verified via API: "${lesson}" is no longer enrolled!`);
        return {
          success: true,
          action: 'CANCEL',
          status: REGISTRATION_STATUS.CANCELLED,
          ...
          teacher: course.teacherNickName || course.teacherName || 'N/A',
          room: course.room || 'N/A',
          message: 'Class cancelled successfully (verified via campus API)!',
        };
      }
    } catch (checkErr) {
      console.debug('[Cancel] Post-cancellation API check skipped:', checkErr);
    }
