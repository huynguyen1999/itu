...
export function resolveTargetWeeks(options: TargetWeekOptions = {}, anchorDate = new Date()): TargetWeeksResult {
  ...
  // If raw is a date string YYYY-MM-DD (e.g. 2026-08-17)
  if (/^\d{4}-\d{2}-\d{2}$/.test(raw)) {
    const monday = getMondayOfWeek(raw);
    return {
      weekMode: WEEK_MODES.CUSTOM,
      weekLabel: `Week of ${monday} (${raw} to ${getSundayOfWeek(monday)})`,
      mondays: [monday],
      primaryMonday: monday,
    };
  }
  ...
  const prevMonday = formatLocalDate(new Date(ty, tm - 1, td - 7));
  const nextMonday = formatLocalDate(new Date(ty, tm - 1, td + 7));

  if (raw === 'prev' || raw === 'previous' || raw === 'prev-week') {
    return {
      weekMode: WEEK_MODES.PREV,
      weekLabel: `Previous Week (${prevMonday} to ${getSundayOfWeek(prevMonday)})`,
      mondays: [prevMonday],
      primaryMonday: prevMonday,
    };
  }
...
export async function fetchMultiWeekSchedule(
  ...
  if (Array.isArray(options.toCancelCourses)) {
    for (const course of options.toCancelCourses) {
      if (course.date && /^\d{4}-\d{2}-\d{2}$/.test(course.date)) {
        datesToFetch.add(getMondayOfWeek(course.date));
      }
    }
