...
import type { ApiClientOptions, RegisteredListResponse, WeekScheduleData } from './types.ts';
...
export class TalkFirstApiClient {
  ...
  /**
   * Fetches the registered/enrolled classes list from TalkFirst for a given date range.
   */
  async fetchRegisteredClasses({
    ...
    endDate: string;
    type?: string;
  }): Promise<RegisteredListResponse> {
    if (!startDate || !endDate) {
      throw new TalkFirstApiError('[TalkFirstApiClient] Both startDate and endDate (YYYY-MM-DD) are required.');
    }
    const query = new URLSearchParams({ type, startDate, endDate }).toString();
    return await this.sendRequest<RegisteredListResponse>(`${API_ROUTES.STUDENT_REGISTERED_LIST}?${query}`, {
      method: 'GET',
      headers: {
        Referer: `${this.baseUrl}${WEB_ROUTES.REGISTERED_LIST}`,
      },
    });
  }
}
