...
- Calendar exposes its existing cache projection through a public feature entrypoint; Planning now owns task queries, selection, grouping, ordering, composition, and sync policy in focused modules; macOS `APIClient` now keeps transport/session/auth state in a 15 KB actor core while endpoint families live in `APIClient+*.swift` extensions.
- Planning task detail editing keeps mutation ownership in `TaskDetailModal` while subtasks and tags render through the focused `TaskDetailSections` feature component.
- Focus keeps timer/session mutations in `FocusPage`; timer/audio/task controls live in `FocusTimerCard`, history browsing lives in `FocusRecordsCard`, and record editing is presented by `FocusRecordEditorDialog`. `FocusPage.tsx` is approximately 21.1 KB, down from 54.7 KB.
- Deck details keeps query/mutation orchestration in `DeckDetailPage`; deck editing, AI card generation, card browsing/selection, stats, and move-dialog behavior live in focused deck components. `DeckDetailPage.tsx` is approximately 14.9 KB, down from 49.5 KB.
- Active Gym workouts keep query/mutation and sync policy in `ActiveWorkoutPage`; exercise-library search/filter/custom creation lives in `ExercisePickerDialog`, and exercise/set editing lives in `WorkoutExerciseList`. `ActiveWorkoutPage.tsx` is approximately 23.7 KB, down from 44.8 KB.
- Statistics keeps date-range/query orchestration in `StatisticsPage`; foreground app activity, website drill-down state, and their loading/error surfaces live in focused statistics components. `StatisticsPage.tsx` is approximately 24.5 KB, down from 53.9 KB.
- Habits keeps weekly grouping, occurrence actions, and page composition in `HabitsPage`; create/edit dialogs live in `HabitEditor` and `HabitDetail`, with shared tag/metric fields and habit date projection kept feature-local. The existing `HabitDetail` public export remains intact. `HabitsPage.tsx` is approximately 20.6 KB, down from 47.8 KB.
- Gym's exercise library keeps filtering and selection in `ExerciseLibraryPage`; creation/upload lives in `CreateExerciseForm`, while editing and performance history live in `ExerciseInspector`. `ExerciseLibraryPage.tsx` is approximately 4.5 KB, down from 30.4 KB.
- Daily journal review keeps save and AI-generation orchestration in `DailyReviewPage`; ledger and AI-insights presentation live in focused journal components. `DailyReviewPage.tsx` is approximately 13.2 KB, down from 26.1 KB.
- Weekly journal review keeps period selection and save/AI-generation orchestration in `WeeklyReviewPage`; weekly metrics and comparisons live in `WeeklyReviewLedger`. `WeeklyReviewPage.tsx` is approximately 23.9 KB, down from 31.6 KB.
- Settings keeps section selection and mutation orchestration in `SettingsPage`; usage controls and device/permission controls live in focused settings components. `SettingsPage.tsx` is approximately 21.5 KB, down from 35.3 KB.
- Shared shell keeps navigation/layout composition in `Layout.tsx`; sync reconciliation and notification querying/actions now live in `SyncStatus.tsx` and `NotificationMenu.tsx`. `Layout.tsx` is approximately 17.9 KB, down from 33.2 KB; the existing helper exports remain re-exported for compatibility.
- Growth reward editing keeps pure weight/draft calculations in the existing `growthRewardMath.ts`, while collapsed reward presentation lives in `GrowthRewardSummary.tsx`. `GrowthRewardEditor.tsx` is approximately 21.3 KB, down from 29.2 KB, with its existing helper re-exports preserved.
- API Growth awards keep transactional ledger/inventory orchestration in `growth-awards.ts`; deterministic scaling, budget allocation, weighted routing, and lifecycle parsing live in `growth-award-calculations.ts`. `growth-awards.ts` is approximately 23.3 KB, down from 26.9 KB, with public award entrypoints unchanged.
- API Growth reward/rule persistence now lives in `PrismaGrowthRewards`; `PrismaGrowthRepository` remains the stable facade and continues to compose reset and shop persistence. The facade is approximately 21.4 KB, down from 34.8 KB, and all Growth repository call sites remain unchanged.

## Completed: sync, Planning, and OfflineStore concentration refactor

- Shared web sync now owns lifecycle, transport, queues, conflicts, persistence, and generic cache reconciliation only. Queue orchestration is separated from pure retry, conflict, cursor, and coalescing policy. Growth receipt interpretation and optimistic receipt projection are feature-owned; Task → Focus completion is now an atomic server-side invariant rather than a web sync side effect.
- `PlanningPage.tsx` is approximately 8.7 KB and composes header, composer, bulk actions, workspace, detail, context, and undo surfaces. Its existing filtering, creation, selection, grouping, ordering, pagination, and modal behavior remains covered by the planning and full web suites.
- macOS `OfflineStore` keeps its unified actor facade while Gym writes live in exercise/workout/nested-set extensions. Hydration is a coordinator over task/list, deck, habit, and Growth replay extensions; no persistence schema or call-site surface changed.
- Signed macOS build and focused `OfflineStoreTests` pass. SwiftLint and Periphery remain non-blocking and skip locally because neither optional tool is installed.
