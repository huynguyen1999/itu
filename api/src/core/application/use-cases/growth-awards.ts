...
/**
 * Account XP is an event budget, not a per-selected-skill multiplier. Study
 * and focus have bounded source semantics that are intentionally independent
 * of the coin/item rule caps: one XP per reviewed pair (up to 20), and one XP
 * per five valid focus minutes (up to 15).
 */
function scaledAccountXp(
  xpReward: number,
  scalingMode: GrowthScalingMode,
  maxRewardCap: number | null,
  sourceType: GrowthSourceType,
...
/** Allocate the fixed account budget using stable largest-remainder rounding. */
function allocateSkillBudget(
  budget: number,
...
function allocateWeightedAmount(
  amount: number,
  mappings: AttributeMappingWithAttribute[],
): number[] {
...
function lifecycleOrdinal(entryKey?: string | null): number {
