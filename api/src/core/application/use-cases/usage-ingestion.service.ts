...
export class UsageIngestionService {
  ...
  async replaceBatch(userId: string, input: UsageSummaryBatchInput) {
    ...
    if (input.summaries.length === 0 && device.platform !== 'MACOS') {
      throw new InvalidRequestException('Usage summaries require a macOS Sync Device');
    }
    ...
    for (const rawSummary of input.summaries) {
      ...
      if (!USAGE_SOURCES.includes(source)) throw new InvalidRequestException('source is invalid');
      if (source === 'DEVICE_ACTIVITY' && device.platform !== 'IOS') {
        throw new InvalidRequestException('DeviceActivity usage requires an iOS Sync Device');
      }
      if (source === 'MACOS_FOREGROUND' && device.platform !== 'MACOS') {
        throw new InvalidRequestException('Foreground usage requires a macOS Sync Device');
      }
      if (source === 'BROWSER' || source === 'HEALTH_KIT') {
        throw new InvalidRequestException('This source is not valid for app summaries');
      }
      ...
      if (typeof summary.bundleId !== 'string' || !summary.bundleId || summary.bundleId.length > 255)
        throw new InvalidRequestException('bundleId is required and must be at most 255 characters');
      if (typeof summary.displayName !== 'string' || !summary.displayName || summary.displayName.length > 255)
        throw new InvalidRequestException('displayName is required and must be at most 255 characters');
      if (
      ...
      if (
        ...
        throw new InvalidRequestException('engagedSeconds must be an integer between 0 and activeSeconds');
      if (summary.hour !== undefined && (!Number.isInteger(summary.hour) || summary.hour < 0 || summary.hour > 23))
        throw new InvalidRequestException('hour must be an integer between 0 and 23');
      for (const [name, value] of [
        ['pickups', summary.pickups],
        ['notifications', summary.notifications],
      ] as const) {
        if (value !== undefined && (!Number.isInteger(value) || value < 0)) {
          throw new InvalidRequestException(`${name} must be a nonnegative integer`);
        }
      }
  ...
  async ingestScreenTimeEvents(userId: string, input: ScreenTimeUsageBatchInput) {
    if (!input.collectorDeviceId || !Array.isArray(input.events) || input.events.length > MAX_BATCH_SIZE) {
      throw new InvalidRequestException(`events must contain at most ${MAX_BATCH_SIZE} entries`);
    }
    const collector = await this.usage.findDevice(userId, input.collectorDeviceId);
    if (!collector) throw new ForbiddenResourceException('Collector device does not belong to this user');
    if (collector.platform !== 'MACOS') {
      throw new InvalidRequestException('Screen Time events require a macOS collector Sync Device');
    }
    ...
    for (const raw of input.events) {
      ...
      if (source !== 'SCREEN_TIME_BIOME') {
        throw new InvalidRequestException('Screen Time events must use SCREEN_TIME_BIOME source');
      }
      if (typeof raw.eventId !== 'string' || !raw.eventId || raw.eventId.length > 128) {
        throw new InvalidRequestException('eventId is required and must be at most 128 characters');
      }
      if (typeof raw.sourceDeviceId !== 'string' || !raw.sourceDeviceId || raw.sourceDeviceId.length > 128) {
        throw new InvalidRequestException('sourceDeviceId is required and must be at most 128 characters');
      }
      ...
      if (
        ...
        throw new InvalidRequestException('durationSeconds must be an integer between 0 and 86400');
      }

      const startedAt = new Date(raw.startedAt);
      const endedAt = new Date(raw.endedAt);
      if (Number.isNaN(startedAt.getTime()) || Number.isNaN(endedAt.getTime())) {
        throw new InvalidRequestException('startedAt and endedAt must be valid ISO date strings');
      }
      if (startedAt > endedAt) {
        throw new InvalidRequestException('startedAt must be before or equal to endedAt');
      }
      ...
      if (raw.durationSeconds > spanSeconds + USAGE_CONSTANTS.screenTimeToleranceSeconds) {
        throw new InvalidRequestException('durationSeconds exceeds interval length plus tolerance');
      }
      ...
      parsedEvents.push({
        ...
        sourceDeviceName: raw.sourceDeviceName && raw.sourceDeviceName.trim() !== '' ? raw.sourceDeviceName.trim().slice(0, 255) : null,
