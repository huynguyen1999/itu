...
- Signature gradients: primary actions use a teal→deep-teal gradient with a mint glow; Growth surfaces add restrained gold and violet atmosphere; page canvases wear an ambient radial wash
...
Both clients expose the same workspace language: Home, Plan, Matrix, Focus, Calendar, Habits, Statistics, Budget, Gym, Journal, Learn, Growth, Trash, Conflicts, Notifications, Profile, and Settings. Web keeps Conflicts, Notifications, Profile, and Settings in utility access rather than the primary workspace groups. Plan owns Inbox, Today, Next 7 Days, and completed/task-list context; Learn owns Decks & cards, Review, and Learning history; Budget owns Overview, Transactions, Budgets, and Categories; Gym owns Overview, active workout continuation, History, and Exercises. Calendar is a shared Day/Week/Month timeline for Tasks, Due Dates, Focus Sessions, and read-only external events.
...
- **Seafoam Mint Accent** (`#167f71` / `--itu-teal-600`, macOS `teal`): The active primary accent for key actionable buttons, focus session banners, progress indicators, and active states. In dark mode this role lightens to `#40a6b5` for contrast against the forest surface.
...
- **Victory Gold** (`#ad8a3d` / macOS `gold`): Restrained Growth and premium reward accent; macOS also exposes `goldSoft` (`#f1e7cf`). On web, `--itu-glow-gold` is a compatibility alias for the mint glow rather than a solid gold fill.
...
| Primary Teal | `--itu-teal-600` | `teal` | `#167f71` | `#40a6b5` |
...
Dark-mode `--primary` and `--ring` use the lifted teal; the dark-mode flat primary uses a near-black teal foreground (`hsl(166 64% 9%)`). The gradient primary action button keeps light text (`#f4faf7`) in both modes.
...
**The Lifted Accent Rule.** In dark mode the primary accent lifts from deep teal (`#167f71`) to `#40a6b5` so actions remain legible on the forest surface; flat primary fills invert their foreground to a near-black teal.
...
- **Calendar workspace**: bounded page shell with source-grouped Day/Week/Month timeline views, a `60px` hourly Day/Week grid, `112px` Month date columns, and a settings surface for visible kinds, completed visibility, week start, and external calendars.
...
Forms are characterized by smooth, friendly curvature. Web core surfaces use the `10px` / `14px` / `18px` radius scale; Tailwind's bridge also supplies `8px` / `10px` / `12px` utility radii. macOS uses `.continuous` corners with shared controls at roughly `8px`–`14px` and the native gradient-card helper defaulting to `20px` for hero surfaces.
...
### Named Rules
**The Three-Tier Rule.** Web feature surfaces stay within the `s` (`10px`), `m` (`14px`), and `l` (`18px`) scale, with pills (`999px`) reserved for chips and status. Native macOS controls may use the platform's continuous corner treatment and the shared hero-card exception.
...
- **Primary (default):** Gradient surface `.itu-primary-action` — teal→deep-teal `linear-gradient(145deg, #218f7e, #12685d 52%, #0b443c)` with a radial mint highlight at the top-right, light text `#f4faf7`, layered lift shadow. Hover brightens (`filter: brightness(1.08)`), active nudges down 1px.
...
- **Deep Gradient Card** (`.itu-gradient-card`): `--itu-gradient-deep` (`linear-gradient(165deg, #0d3831, #0a2b26)`) with the mint glow layer (`--itu-glow-mint`), light text `#edf3f0` — used for journal, focus, and summary hero surfaces.
...
- **Web navigation behavior:** Primary workspace items are fixed in four semantic groups: Productivity, Tracking, Knowledge, and System. Calendar sits in Productivity; Conflicts, Notifications, Profile, Settings, theme, and sync remain utility access in the rail footer, compact header, or mobile More menu.
...
- **Calendar Timeline** ([`CalendarTimeline.tsx`](web/src/features/calendar/components/CalendarTimeline.tsx)): Source-grouped Day/Week/Month timeline with sticky headers, all-day and spanning items, collision-aware placement, read-only details for Focus Sessions and external events, and drag/move/resize affordances for Tasks.
...
- **Don't** add one-off semantic colors. If a new state needs color, assign it a documented role and mirror that role in the web CSS tokens and macOS theme before using it.
