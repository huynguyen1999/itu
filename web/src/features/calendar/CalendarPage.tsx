...
function CalendarGroupRow({ group, days, zoom, width, collapsed, onToggle, onSelect, onDragStart, onResize, onResizeStep }: { group: CalendarGroup; days: Date[]; zoom: TimelineZoom; width: number; collapsed: boolean; onToggle: () => void; onSelect: (item: CalendarTimelineItem) => void; onDragStart: (item: CalendarTimelineItem, event: React.DragEvent<HTMLElement>) => void; onResize: (item: CalendarTimelineItem, edge: 'start' | 'end', event: React.PointerEvent) => void; onResizeStep: (item: CalendarTimelineItem, edge: 'start' | 'end', direction: -1 | 1) => void }) {
  return (
    <div className="border-b border-border/70">
      <div
        className="flex items-center gap-2 cursor-pointer select-none border-b border-border/40 bg-card/50 px-4 py-2 hover:bg-card/80 transition-colors"
        onClick={onToggle}
        role="button"
        tabIndex={0}
      ...
      >
        <span className="h-2 w-2 rounded-full flex-none" style={{ backgroundColor: group.color }} />
        <span className="text-xs font-semibold text-foreground">{group.label}</span>
        <span className="font-mono text-[10px] text-muted-foreground">{group.subtitle}</span>
        <ChevronDown className={`ml-auto h-3.5 w-3.5 text-muted-foreground transition-transform duration-200 ${collapsed ? '-rotate-90' : ''}`} />
      </div>
      {collapsed ? (
        <div className="h-12 bg-[var(--itu-surface-2)]" />
      ) : (
        <div style={{ width }}>
          {zoom === 'DAY'
            ? <DayRow group={group} day={days[0] ?? new Date()} onSelect={onSelect} onDragStart={onDragStart} onResize={onResize} onResizeStep={onResizeStep} />
            : <DateRow group={group} days={days} zoom={zoom} onSelect={onSelect} onDragStart={onDragStart} onResize={onResize} onResizeStep={onResizeStep} />}
        </div>
      )}
    </div>
  );
...
function DayRow({ group, day, onSelect, onDragStart, onResize, onResizeStep }: { group: CalendarGroup; day: Date; onSelect: (item: CalendarTimelineItem) => void; onDragStart: (item: CalendarTimelineItem, event: React.DragEvent<HTMLElement>) => void; onResize: (item: CalendarTimelineItem, edge: 'start' | 'end', event: React.PointerEvent) => void; onResizeStep: (item: CalendarTimelineItem, edge: 'start' | 'end', direction: -1 | 1) => void }) {
  ...
  return <div className="relative bg-[var(--itu-surface-2)]" style={{ height: rowHeight }}><div className="absolute inset-x-0 top-0 flex flex-col gap-1 overflow-hidden border-b border-border/60 bg-card/70 px-3" style={{ height: allDayHeight }}>{allDayItems.map((item) => <CalendarItem key={item.id} item={item} left={0} top={0} onSelect={onSelect} onDragStart={onDragStart} onResize={onResize} onResizeStep={onResizeStep} compact />)}</div><div className="absolute inset-x-0 bottom-0" style={{ top: allDayHeight }}>{Array.from({ length: 24 }, (_, hour) => <div key={hour} className="absolute inset-y-0 border-l border-border/40" style={{ left: hour * DAY_HOUR_WIDTH }} />)}{timedItems.map((item, index) => {
    const itemStart = new Date(item.startAt);
    const itemEnd = item.endAt ? new Date(item.endAt) : new Date(itemStart.getTime() + 30 * 60_000);
    const effStart = itemStart < dayStart ? dayStart : itemStart;
    const effEnd = itemEnd > dayEnd ? dayEnd : itemEnd;
    const startMinutes = (effStart.getTime() - dayStart.getTime()) / 60_000;
    const durationHours = Math.max(0.5, (effEnd.getTime() - effStart.getTime()) / 3_600_000);
    return <CalendarItem key={item.id} item={item} left={(startMinutes / 60) * DAY_HOUR_WIDTH} top={timedTops[index]} width={Math.max(64, durationHours * DAY_HOUR_WIDTH)} onSelect={onSelect} onDragStart={onDragStart} onResize={onResize} onResizeStep={onResizeStep} />;
  })}</div></div>;
}
...
/** Original absolute-positioned item used in the DAY view. */
function CalendarItem({ item, left, top, width, compact, onSelect, onDragStart, onResize, onResizeStep }: { item: CalendarTimelineItem; left: number; top: number; width?: number; compact?: boolean; onSelect: (item: CalendarTimelineItem) => void; onDragStart: (item: CalendarTimelineItem, event: React.DragEvent<HTMLElement>) => void; onResize: (item: CalendarTimelineItem, edge: 'start' | 'end', event: React.PointerEvent) => void; onResizeStep: (item: CalendarTimelineItem, edge: 'start' | 'end', direction: -1 | 1) => void }) {
  ...
  return (
    <div
      ...
      className={`group ${compact ? 'relative min-w-0 flex-1' : 'absolute'} overflow-hidden text-left outline-none transition-all duration-150 hover:-translate-y-[1px] hover:shadow-lg focus-visible:z-20 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 rounded-[9px] border border-white/20 pl-[12px] pr-[10px] py-[6px] text-foreground ${item.readOnly ? 'opacity-90 cursor-default' : 'cursor-grab active:cursor-grabbing'}`}
      style={{
        left,
        top,
        ...(width === undefined ? {} : { width }),
        minHeight: item.allDay ? 26 : 48,
        backgroundColor: color.startsWith('#') ? `${color}24` : color.startsWith('var') ? `rgba(45, 212, 191, 0.14)` : `${color}`,
        borderColor: color.startsWith('#') ? `${color}60` : color.startsWith('var') ? `rgba(45, 212, 191, 0.38)` : `rgba(255, 255, 255, 0.25)`,
      }}
    ...
    >
      ...
      {!item.allDay ? (
        hasDuration ? (
          sameDay ? (
            <div className="mt-0.5 block font-mono text-[9.5px] leading-snug text-muted-foreground whitespace-nowrap overflow-hidden">
          ...
          <span className="mt-0.5 block font-mono text-[9.5px] leading-snug text-muted-foreground whitespace-nowrap overflow-hidden">
            {formatSingleTime(item.startAt)}
          </span>
        )
      ) : null}
      {item.kind === 'TASK_DURATION' && !item.readOnly ? (
        <>
...
/** Board-style card used in the WEEK / MONTH column view. Items stack vertically per day. */
function BoardCalendarItem({ item, onSelect, onDragStart, onResize, onResizeStep }: { item: CalendarTimelineItem; onSelect: (item: CalendarTimelineItem) => void; onDragStart: (item: CalendarTimelineItem, event: React.DragEvent<HTMLElement>) => void; onResize: (item: CalendarTimelineItem, edge: 'start' | 'end', event: React.PointerEvent) => void; onResizeStep: (item: CalendarTimelineItem, edge: 'start' | 'end', direction: -1 | 1) => void }) {
  const color = timelineItemColor(item.kind, item.color);
  const label = item.kind === 'TASK_DUE' ? 'Due Date' : item.kind === 'FOCUS_SESSION' ? 'Focus Session' : item.kind === 'EXTERNAL_EVENT' ? 'Subscription' : 'Task';
  const hasDuration = Boolean(item.endAt) && !item.allDay;
  const sameDay = hasDuration && isSameLocalDay(item.startAt, item.endAt!);

  // Resolve tint and border colors from the accent color
  ...
  return (
    <div
      ...
      aria-label={`${item.title}, ${label}${item.readOnly ? ', read-only' : ', draggable'}`}
      className={`group relative w-full overflow-hidden rounded-[10px] border pl-[14px] pr-[10px] py-[10px] text-left outline-none transition-all duration-150 hover:brightness-110 hover:-translate-y-px focus-visible:z-20 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 ${
        item.readOnly ? 'cursor-default' : 'cursor-grab active:cursor-grabbing'
      }`}
      style={{ backgroundColor: bgColor, borderColor }}
    >
      {/* Left accent rail */}
      <div className="absolute inset-y-0 left-0 w-[3.5px] rounded-l-[10px]" style={{ backgroundColor: color }} />
      ...
      {/* Time block — only when the item has time information */}
      {!item.allDay && (hasDuration || item.startAt) ? (
        <div
          className="mt-2 flex flex-col items-start gap-px border-t pt-2 font-mono"
          style={{ borderColor: 'rgba(255,255,255,0.08)' }}
        >
          <span className="text-[11px] leading-snug text-muted-foreground">{formatSingleTime(item.startAt)}</span>
          {hasDuration && sameDay ? (
            <>
              <span className="text-[9px] leading-none text-muted-foreground/50 ml-0.5">↓</span>
      ...
      {/* Resize handles for TASK_DURATION in week/month — repurposed as top/bottom */}
      {item.kind === 'TASK_DURATION' && !item.readOnly ? (
        <>
          <button
            type="button"
            ...
            onPointerDown={(e) => onResize(item, 'start', e)}
            onKeyDown={(e) => { if (e.key === 'ArrowLeft') onResizeStep(item, 'start', -1); if (e.key === 'ArrowRight') onResizeStep(item, 'start', 1); }}
            className="absolute inset-x-0 top-0 h-1.5 cursor-ns-resize opacity-0 group-hover:opacity-100 focus-visible:opacity-100"
          />
          <button
            type="button"
            aria-label={`Resize end of ${item.title}`}
            onClick={(e) => e.stopPropagation()}
            onPointerDown={(e) => onResize(item, 'end', e)}
            onKeyDown={(e) => { if (e.key === 'ArrowLeft') onResizeStep(item, 'start', -1); if (e.key === 'ArrowRight') onResizeStep(item, 'end', 1); }}
            className="absolute inset-x-0 bottom-0 h-1.5 cursor-ns-resize opacity-0 group-hover:opacity-100 focus-visible:opacity-100"
          />
        </>
      ) : null}
    </div>
